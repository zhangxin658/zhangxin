    ...
    def __init__(self, name, DNA_size):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('mean'):
                self.mean = tf.Variable(tf.truncated_normal([self.DNA_size, ], stddev=0.05, mean=0.5), dtype=tf.float16,
                                        name=name + '_mean')
            with tf.variable_scope('cov'):
                self.cov = tf.Variable(1.0 * tf.eye(self.DNA_size), dtype=tf.float16, name=name + '_cov')
...
class Worker_pop(object):
    def __init__(self, name, DNA_size, global_pop, N_POP_size, Factor=2):
        ...
        with tf.variable_scope(name):
            ...
            self.tfkids_fit = tf.placeholder(tf.float16, [self.C_POP_size, ])
            self.tfkids = tf.placeholder(tf.float16, [self.C_POP_size, self.DNA_size])
            ...
            with tf.name_scope('restart'):
                self.re_mean_op = self.mean.assign(
                    tf.Variable(tf.truncated_normal([self.DNA_size, ], stddev=0.05, mean=0.5), dtype=tf.float32))
...
class Dataset(object):
    ...
    def __loadData__(self, label_loc, div_mode, test_size, select_feature_size):
        ...
        if self.label_loc == 0:
            ...
            data = pd.read_table(self.filenamed, sep=' ')
            self.fea, self.lab = data.ix[:, 0:len(open(self.filenamed).readline().split(self.div_mode)) - 1], data.ix[:, -1]
            self.DNA = len(np.array(self.fea)[0])
            if self.DNA < self.select_feature:
                self.select_feature = self.DNA
    ...
    def __getData__(self):
        ...
        if self.type == 'split':
            self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(self.train_selected, self.lab,
...
'''
class Worker(object):
    def __init__(self, name, pop, sub, data, classifier, factor, block):
        self.name = name
        self.pop = pop
        self.sub = sub
        self.data = data
        ...
        self.factor = factor
        self.Block = block
        self.global_pop = Global_pop(name, )
        self.popnet = Worker_pop(name, data, global_pop, 0.5)  # 每个线程在初始化中首先区初始化一个网络

    # def build_global_pop(self, name, subSize):
    #     if
...
if __name__ == '__main__':
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        N_WORKERS = multiprocessing.cpu_count()-4   # 表示运行的线程个数
        N_POP = 30    # 表示生成子代的个数
        ...
        Factor = 2
        Block = 2
        data = Dataset('D:/Dataset/Vehicle', SPLIT)
                ...
                for sub in range(Block):
                    with tf.device('/gpu:%d' % i):
                        ...
                        with tf.name_scope('sub_%d' % (sub + 1)) as scope:
                            ...
                            workers.append(Worker(i_name, pop, sub, data, SVM, Factor, Block))
